...
Aviso que não cabe esconder, e que neste acervo pesa mais que nos outros: **a
aprovação é automática**. Quem chegar à URL completa o fluxo e recebe um token.
Isso protege contra chamada sem token, não contra quem conhece o endereço.

Nos acervos públicos deste repositório isso era aceitável — Diário Oficial e
legislação municipal a Prefeitura já publica na internet, e o pior que
acontece é alguém ler o que já era lido. Aqui não é o mesmo caso. Os lotes vêm
de portal público, mas a LISTA DE OPORTUNIDADES não: ela é a sua leitura de
mercado, e quem a obtiver dá lance contra você no mesmo pregão. Trate a URL
como credencial, não a publique, e prefira mantê-la fora de índice de busca.

O segredo de assinatura vem de `LEILOES_SEGREDO_OAUTH` — o nome está em
VARIAVEL_DO_SEGREDO, abaixo, e é dali que sai a mensagem de aviso. Sem ele,
cada partida do processo sorteia um novo, e toda autorização concedida antes
deixa de valer, o que na hospedagem gratuita significa reautorizar depois de
cada hibernação.
"""
...
ESCOPO = "leiloes-numismatica"

# O nome da variável de ambiente que ESTE servidor lê, como constante e não
# escrito à mão dentro da mensagem de aviso.
#
# Isto é a correção de uma cicatriz que a cópia anterior deste módulo carrega
# documentada: ela veio do acervo do Ementário, o aviso continuou dizendo
# `EMENTARIO_SEGREDO_OAUTH`, e quem o seguisse definiria uma variável que o
# servidor nunca leu — o alerta seguiria aparecendo, sem explicação. Como este
# arquivo é a TERCEIRA cópia, o nome saiu de dentro do texto e virou constante,
# e há um teste que confere que o aviso nomeia a variável que o `__main__`
# de fato lê. Da próxima cópia, o teste falha antes do usuário perceber.
VARIAVEL_DO_SEGREDO = "LEILOES_SEGREDO_OAUTH"
...
class ProvedorOAuth(OAuthAuthorizationServerProvider):
    ...
    async def revoke_token(self, token: AccessToken | RefreshToken) -> None:
        ...
        troque a variável nomeada em VARIAVEL_DO_SEGREDO: as assinaturas
        antigas param de valer todas de uma vez.
        """
...
def montar(url_publica: str, segredo: str | None) -> tuple[ProvedorOAuth, AuthSettings]:
    ...
    if not segredo:
        ...
        # O nome da variável neste aviso é o que quem opera o serviço vai
        # procurar para definir. Por isso ele vem de VARIAVEL_DO_SEGREDO, e não
        # escrito aqui: escrito aqui foi como a cópia anterior passou a apontar
        # para uma variável que o servidor não lia.
        log.warning(
            "%s não definido: usando um segredo temporário. As autorizações "
            "concedidas agora deixam de valer quando o serviço reiniciar — e no "
            "plano gratuito ele reinicia a cada hibernação.", VARIAVEL_DO_SEGREDO
        )
